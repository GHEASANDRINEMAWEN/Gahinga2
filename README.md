...
* `src/:` Contains the source code for the application, including both Java and Bash scripts.

    * `src/Bash/:` Houses the Bash scripts responsible for user and patient management, including functions like login, registration, and data processing.
    * `src/Storage/:` Stores data files used by the application.
        * `user-store.txt:` Holds user information, such as UUID codes, emails, hashed passwords, access types, and registration status.
        * `patients-store.txt:` Stores patient information, including personal details and medical data.
        * `life-expectancy.csv:` A CSV file containing country codes, names, and life expectancy statistics for various countries.
    * `src/View/:` Contains Java classes responsible for the application's user interface and core functionality.
        * `Test.java:` The main class that serves as the entry point to run the program.
* `README.md:` This file, which provides detailed instructions on how to set up, run, and use the project.
...
### 1. Give bash script accessibility:
```
chmod +777 src/Bash/user-manager.sh
```

### 2. Compile the Java Code:
```
javac -d bin src/Model/*.java src/View/Test.java
```

### 3. Run the Program:
```
java -cp bin View.Test
```
